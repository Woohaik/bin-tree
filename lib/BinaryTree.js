...
var BinaryTree = /** @class */ (function () {
    ...
    BinaryTree.prototype.contain = function (value) {
        var theSet = __spread(new Set(value.sort(function (a, b) { return a - b; })));
        var realValues = this.inorder();
        // For an array where all numbers math the length should not change at filter comparing each one to real values of the tree
        return theSet.filter(function (theValue) { return realValues.filter(function (realValue) { return realValue === theValue; }).length === 1; }).length === theSet.length;
    };
    /*INTERNAL FUNCTIONS */
